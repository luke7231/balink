...
스케줄(요일·시간대)은 **LLM이 제목+본문을 먼저 읽고 확정**한다. 룰은 sanitize/검증만 한다.

```json
{
  "days": ["월", "화", "수", "목", "금", "토"],
  "dayGroups": [["월", "수", "금"], ["화", "목"], ["토"]],
  "dayRaw": "월수금/화목/토",
  ...
  "evidence": "월수금 5-5:50 … 토 오전10-10:50"
}
```
...
- `월`, `화`, `수`, `목`, `금`, `토`, `일`
- `dayGroups`는 **지원 단위**: 그룹 안은 AND(한 사람이 모두), 그룹끼리는 OR(택1).
- **기본은 한 그룹**. 요일별 수업이 나열돼도 한 포지션이면 합친다. 예: `월수 19:00` + `목 19:30` → `[["월","수","목"]]`.
- **진짜 대안**일 때만 분리: 요일묶음 사이 `/`, `또는`, `택1`. 예: `월수금/화목/토` → 세 그룹.
- **한요일만 지원가능**이면 각 요일 단독 그룹. 예: 화·목 + `(한요일만 지원가능)` → `[["화"],["목"]]`.

`timeSlots` 값 (보정된 시각 **구간 겹침**으로 부여):
...
- `1~8시`는 기본 +12(PM, 저녁). raw에 `오전`이 있으면 AM.
- `9~11시`는 기본 오전/낮. 저녁·오후 단서만 있고 오전이 없을 때만 +12.
- 제목의 `저녁 수업` 등도 문맥으로 사용한다.
