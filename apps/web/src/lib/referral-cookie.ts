...
import { CLAIM_INVITE_COOKIE, INVITE_REF_COOKIE } from "@/lib/referral-cookie-name";

export { CLAIM_INVITE_COOKIE, INVITE_REF_COOKIE } from "@/lib/referral-cookie-name";
...
export async function clearClaimInviteCookie(): Promise<void> {
  const jar = await cookies();
  jar.delete(CLAIM_INVITE_COOKIE);
}
