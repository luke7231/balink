...
const inputPath = args.input || path.join("data", "esangdance-employ-2026-06-11.json");
const outputPath =
...
async function main() {
  ...
  for (const listing of listPayload.listings) {
    ...
    try {
      detail = await fetchDetail(listing.url, cookie);
    } catch (error) {
      detailError = error;
      detail = {
        title: listing.title,
    ...
    const raw = {
      ...
      company: listing.writer,
      companyType: null,
      postedDate: listing.postedDate,
      closingDateText: null,
      summaryMajorText: listing.category,
      summaryRegionText: detail.regionText,
      summaryPayText: detail.payText,
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--input") parsed.input = argv[++index];
      ...
      console.log([
        "Usage: npm run classify:esangdance -- --input data/esangdance-employ-2026-06-11.json",
        "",
        "Options:",
...
async function login() {
  const id = process.env.ESANG_DANCE_ID;
  const password = process.env.ESANG_DANCE_PW;

  if (!id || !password) {
    throw new Error("ESANG_DANCE_ID and ESANG_DANCE_PW are required in .env");
  }

  const params = new URLSearchParams({
    mb_id: id,
    mb_password: password,
    url: `${BASE_URL}/M01?sca=${encodeURIComponent("발레")}`,
  });
  const response = await fetchWithRetry(`${BASE_URL}/bbs/login_check.php`, {
    method: "POST",
    redirect: "manual",
    headers: {
  ...
  if (!cookie) throw new Error("Failed to create Esang Dance login session.");
  return cookie;
}
...
function buildCookieHeader(headers) {
  ...
  for (const setCookie of setCookies) {
    ...
    if (!name || value === "deleted") continue;
...
async function fetchDetail(url, cookie) {
  ...
  const title = cleanText($("blockquote h4").first().text()).replace(/^\[[^\]]+\]\s*/, "");
  const detailHtml = $("#bo_v_con").first().html() || "";
  const detailImages = $("#bo_v_con img")
    .map((_, image) => ({
      src: $(image).attr("src") || null,
      alt: cleanText($(image).attr("alt") || ""),
    }))
    .get();
  const detailText = cleanDetailText(detailHtml) || detailImages.map((image) => image.alt || image.src).filter(Boolean).join("\n");

  if (/글을 읽을 권한이 없습니다|로그인 후 이용/.test($.text()) || !$("#bo_v_con").length) {
    throw new Error(`Failed to read Esang Dance detail page after login: ${url}`);
  }

  return {
    title,
    regionText: extractLabeledValue(detailText, "학원명(지역)"),
    payText: extractLabeledValue(detailText, "수업료 등 추가내용"),
    detailText,
    detailImages,
  };
}
...
function extractLabeledValue(text, label) {
  ...
  const otherLabels = labels.filter((item) => item !== label).map(escapeRegExp).join("|");
